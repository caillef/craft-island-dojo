use craft_island::models::{Direction, Position, Vec2, Block};

#[dojo::interface]
trait IActions {
    fn spawn(ref world: IWorldDispatcher);
    fn move(ref world: IWorldDispatcher, direction: Direction);
    fn place_block(ref world: IWorldDispatcher, x: u32, y: u32, z: u32, blockType: u32);
    fn remove_block(ref world: IWorldDispatcher, x: u32, y: u32, z: u32);
...
mod actions {
    ...
    use craft_island::models::{
        Position, Moves, Direction, Vec2, PlayerConfig, PlayerItem, ServerProfile, Block
    };
    ...
    impl ActionsImpl of IActions<ContractState> {
        ...
        fn place_block(ref world: IWorldDispatcher, x: u32, y: u32, z: u32, blockType: u32) {
            let player = get_caller_address();
            let CHUNK_SIZE = 6;
            let (cx, cy, cz) = (x / CHUNK_SIZE, y / CHUNK_SIZE, z / CHUNK_SIZE);
            let mut chunk = get!(world, (player, cx, cy, cz), (Chunk));
            let (bx, by, bz) = (x % CHUNK_SIZE, y % CHUNK_SIZE, z % CHUNK_SIZE);
            let id = bx + by * CHUNK_SIZE;
            if id < (CHUNK_SIZE * CHUNK_SIZE) / 2 {} else {}
        // let mut block = get!(world, (x, y, z), (Block));
